...
# Estou usando o Lifespan para substituir o on_event descontinuado
@asynccontextmanager
async def lifespan(app: FastAPI):
    # Executado no STARTUP da aplicação (antes de começar a receber requisições)
    ...
    # Executado no SHUTDOWN da aplicação (ao encerrar o servidor)
    logger.info("Encerrando aplicação: Finalizando recursos pendentes...")

# Inicializando o app passando o ciclo de vida moderno
app = FastAPI(
...
async def transmit_to_sap_worker(log_id: str, company_code: str, session_generator):
    """
    Trabalhador em segundo plano que tenta enviar os dados ao SAP.
    """
